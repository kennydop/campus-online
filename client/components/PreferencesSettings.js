...
function PreferencesSettings() {
  ...
  async function changeTheme(pref){
    setTheme(pref)
    axios.put(`${process.env.NEXT_PUBLIC_SERVER_BASE_URL}/api/users/${currentUser._id}`, { preferences: {theme: pref} }, { headers: { Authorization: `Bearer ${currentUser.token}`}, withCredentials: true, credentials: 'include'})
  }
  ...
  return (
    <div className="w-full h-105 bg-white dark:bg-bdark-100 rounded-b-lg md:rounded-r-lg md:rounded-bl-lg flex flex-col justify-center p-3 shadow-md">
      <div className="block md:hidden text-gray-500 dark:text-gray-400 text-lg font-semibold mb-6 mx-auto">Preferences</div>
