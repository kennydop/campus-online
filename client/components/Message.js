  ...
  return (
    message.from===currentUser._id?
      <div ref={ref} className='self-end fit-content flex flex-col cursor-default'>
        <div onClick={()=>setSeeAdditionalInfo(!seeAdditionalInfo)} className='text-white fit-content rounded-3xl shadow-md bg-pink-500 px-3 py-1 my-0.5 max-w-xs relative whitespace-pre-wrap break-words text-left'>{message.msg}</div>
        <div className={`flex self-end justify-center items-center text-2xs font-semibold text-gray-500 dark:text-gray-400 ${seeAdditionalInfo?'block':'hidden'}`}>
          <TimePast date={new Date(message.sent)} msg _sent/> | <EyeIcon className={`h-3 ml-1 ${message.read &&'text-pink-500'}`}/>
        </div>
      </div>
      :
      <div ref={ref} className='self-start fit-content flex flex-col cursor-default'>
        <div onClick={()=>setSeeAdditionalInfo(!seeAdditionalInfo)} className='text-gray-500 dark:text-gray-400 fit-content rounded-3xl shadow-md bg-white dark:bg-bdark-100 px-3 py-1 my-0.5 max-w-xs relative whitespace-pre-wrap break-words text-left'>{message.msg}</div>
        ...
        <TimePast date={new Date(message.sent)} msg/>
        </div>
      </div>
  )
})
