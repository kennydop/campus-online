...
function SearchChat({hits, clearSearch, setCurrentChat}) {
  ...
  return (
    (hits?.length !== 0 && hits?.length !== undefined) &&
      <div ref={ref} className="flex flex-col mt-3 bg-white dark:bg-bdark-100 rounded-b-lg shadow-all-md dark:shadow-all-lg z-20 text-gray-500 dark:text-gray-400 max-h-96 overflow-y-auto">
        {hits?.map(hit =>
